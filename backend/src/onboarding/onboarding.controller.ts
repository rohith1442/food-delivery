...
export class OnboardingController {
  ...
  @Post('merchant/onboarding/documents') @Roles('MERCHANT') @UseInterceptors(FileInterceptor('file', { storage: memoryStorage(), limits: { fileSize: 5 * 1024 * 1024 } })) async documentMerchant(@Req() req: any, @UploadedFile() file: Express.Multer.File, @Body('type') type: string) { if (!file) throw new BadRequestException('File is required'); const uploaded = await this.kycStorage.upload({ uid: req.user.uid, role: 'MERCHANT', type: type?.toUpperCase(), file }); return this.service.addDocument(req.user.uid, 'MERCHANT', { type, ...uploaded }); }
  ...
  @Post('delivery/onboarding/documents') @Roles('DELIVERY') @UseInterceptors(FileInterceptor('file', { storage: memoryStorage(), limits: { fileSize: 5 * 1024 * 1024 } })) async documentDelivery(@Req() req: any, @UploadedFile() file: Express.Multer.File, @Body('type') type: string) { if (!file) throw new BadRequestException('File is required'); const uploaded = await this.kycStorage.upload({ uid: req.user.uid, role: 'DELIVERY', type: type?.toUpperCase(), file }); return this.service.addDocument(req.user.uid, 'DELIVERY', { type, ...uploaded }); }
