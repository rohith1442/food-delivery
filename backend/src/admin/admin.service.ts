...
async updateZone(
  ...
  if (data.centerLatitude !== undefined) {
    updates.centerLatitude =
      data.centerLatitude;
  }

  if (data.centerLongitude !== undefined) {
    updates.centerLongitude =
      data.centerLongitude;
  }
...
async getDashboard() {
  ...
  const zones = zonesSnapshot.docs.map(
    (doc) => doc.data(),
  );
...
async updateSettings(
  data: Partial<GlobalSettingsDocument>,
) {
  const db =
    this.firebaseService.getFirestore();
  ...
  if (
    ...
    if (
      ...
      throw new BadRequestException(
        'maintenanceMode must be a boolean',
      );
    }
  ...
  if (
    data.customerMinVersion !== undefined
  ) {
    updates.customerMinVersion =
  ...
  if (
    data.merchantMinVersion !== undefined
  ) {
    updates.merchantMinVersion =
      this.validateVersion(
  ...
  if (
    data.deliveryMinVersion !== undefined
  ) {
    updates.deliveryMinVersion =
      this.validateVersion(
