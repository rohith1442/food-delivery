...
class _DeliveryApprovalPendingPageState
  ...
  Future<void> _checkStatus() async {
    ...
    try {
      ...
      if (user == null) {
        _showMessage('Delivery profile not found');
        return;
      }
      ...
      if (role != 'DELIVERY') {
        ...
        _showMessage('This account is not a delivery partner account');
      ...
      if (status == 'PENDING') {
        _showMessage('Your application is still pending approval');

        return;
      }

      if (status == 'REJECTED') {
        _showMessage('Your delivery partner application was rejected');

        return;
      }

      if (status == 'SUSPENDED') {
        _showMessage('Your delivery account is suspended');

        return;
      }

      _showMessage('Your delivery account is not active');
    } on DioException catch (error) {
      final response = error.response?.data;

      String message = 'Unable to check approval status';
      ...
      _showMessage('Unable to check approval status');
    } finally {
      if (mounted) {
        setState(() {
          _loading = false;
